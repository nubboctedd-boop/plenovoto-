const ESTADOS = [
    { id: "PENDIENTE", selector: "estado-PENDIENTE" },
    { id: "A FAVOR", selector: "estado-A-FAVOR" },
    { id: "EN CONTRA", selector: "estado-EN-CONTRA" },
    { id: "ABSTENCION", selector: "estado-ABSTENCION" },
    { id: "NO VOTO", selector: "estado-NO-VOTO" },
    { id: "MESA DIRECTIVA", selector: "estado-MESA-DIRECTIVA" },
    { id: "LICENCIA", selector: "estado-LICENCIA" }
];

let dataCongresistas = [];
let currentMeta = 66;
let currentQuorum = 130;
const STORAGE_KEY = "plenovoto_checkpoint_v2";

document.addEventListener("DOMContentLoaded", () => {
    console.log("🚀 PlenoVoto Pro Iniciado...");
    cargarDatos();
});

function formatNombreOficial(nombreCompleto) {
    if (!nombreCompleto) return '';
    // Limpieza profunda de caracteres especiales de codificación (\uFFFD)
    const clean = nombreCompleto.replace(/\uFFFD/g, 'Í').trim().toUpperCase();
    
    let ape1 = "", initApe2 = "", initNom = "";

    if (clean.includes(',')) {
        const [apellidos, nombres] = clean.split(',').map(s => s.trim());
        const apeArr = apellidos.split(/\s+/);
        const nomArr = nombres.split(/\s+/);
        ape1 = apeArr[0];
        initApe2 = apeArr[1] ? `${apeArr[1][0]}.` : '';
        initNom = nomArr[0] ? `${nomArr[0][0]}.` : '';
    } else {
        const parts = clean.split(/\s+/);
        ape1 = parts[0] || "";
        // Manejar el caso de apellidos compuestos o ruidos
        initApe2 = parts[1] && parts[1].length > 1 ? `${parts[1][0]}.` : "";
        initNom = parts[2] ? `${parts[2][0]}.` : "";
    }

    let res = ape1;
    if (initApe2) res += ` ${initApe2}`;
    if (initNom) res += `, ${initNom}`;
    return res;
}

async function cargarDatos() {
    console.log("📥 Cargando congresistas desde API...");
    try {
        const response = await fetch('/api/congresistas');
        const r = await response.json();
        if (r.data) {
            // Ordenar según prioridad de bancada oficial (según lista proporcionada)
            const bPrioridad = {
                "FP": 1, "APP": 2, "PP": 3, "PL": 4, "RP": 5, 
                "JPP": 6, "SP": 7, "AP": 8, "Av.P": 9, "BS": 10, 
                "HD": 11, "BDP": 12, "N.A.": 13
            };
            
            dataCongresistas = r.data.map(c => ({ ...c, oralizado: false }));
            
            // Ordenamiento exacto: Bancada -> Nombre (Normalizado para AGUINAGA #1)
            dataCongresistas.sort((a, b) => {
                const pA = bPrioridad[a.bancada] || 99;
                const pB = bPrioridad[b.bancada] || 99;
                if (pA !== pB) return pA - pB;
                // Limpiar nombres para comparación alfabética real (\uFFFD)
                const nA = a.nombre.replace(/\uFFFD/g, 'I').trim();
                const nB = b.nombre.replace(/\uFFFD/g, 'I').trim();
                return nA.localeCompare(nB, 'es', { sensitivity: 'base' });
            });

            // LIMITAR A 130 ESCAÑOS OFICIALES
            dataCongresistas = dataCongresistas.slice(0, 130);
            console.log("✅ Tablero ajustado a 130 escaños.");
...
/**
 * LÓGICA DE META (QUÓRUM)
 */
function updateQuorum() {
    const qInput = document.getElementById("quorum-input");
    currentQuorum = parseInt(qInput.value) || 130;
    updateMetaGoal(); // Recalcular meta según nuevo quórum
}

function updateMetaGoal() {
    const selector = document.getElementById("meta-select");
    const customInput = document.getElementById("custom-meta-input");
    const label = document.getElementById("meta-label");
    
    let metaVal = 66;
    let typeLabel = "MAYORÍA SIMPLE";

    if (selector.value === "simple") {
        metaVal = Math.floor(currentQuorum / 2) + 1;
        typeLabel = `MAYORÍA SIMPLE (50%+1 de ${currentQuorum})`;
    } else if (selector.value === "calificada") {
        metaVal = Math.ceil(currentQuorum * 2 / 3);
        typeLabel = `MAYORÍA CALIFICADA (2/3 de ${currentQuorum})`;
    } else if (selector.value === "fija87") {
        metaVal = 87;
        typeLabel = "MAYORÍA LEGAL (87 VOTOS)";
    } else if (selector.value === "custom") {
        customInput.style.display = "inline-block";
        metaVal = parseInt(customInput.value) || 66;
        typeLabel = "META PERSONALIZADA";
    } else {
        customInput.style.display = "none";
    }
    
    currentMeta = metaVal;
    label.innerText = `META: ${currentMeta} (${typeLabel})`;
    updateStats();
}
...
function switchTab(tabId) {
    ...
    // Buscar el botón que corresponde al tabId
    const buttons = document.querySelectorAll('.tab-btn');
    buttons.forEach(btn => {
        if (btn.getAttribute('onclick').includes(tabId)) btn.classList.add('active');
    });
    
    if (tabId === 'tab-cuadrantes') {
        renderHeatmap();
        renderIndividualPoints();
    }
    if (tabId === 'tab-mapa') renderMapa();
    if (tabId === 'tab-analisis-pro') renderAnalysisPro();
}

function switchAnalysisView(viewId) {
    document.querySelectorAll('.analysis-view').forEach(v => v.style.display = 'none');
    document.getElementById(`view-${viewId}`).style.display = 'block';
    if (viewId === 'individual') renderIndividualPoints();
    else renderHeatmap();
}

/**
 * RENDER HEMICICLO (6 COLUMNAS: 23-22-20-20-22-23)
 */
function renderHemiciclo() {
    const loading = document.getElementById("loading");
    if(loading) loading.style.display = "none";
    
    const container = document.getElementById("grid-congresistas");
    container.innerHTML = "";

    // Definición de columnas solicitada
    const colSizes = [23, 22, 20, 20, 22, 23];
    let currentIndex = 0;

    colSizes.forEach((size) => {
        const colDiv = document.createElement("div");
        colDiv.className = "hemiciclo-columna";

        for (let i = 0; i < size; i++) {
            if (currentIndex >= dataCongresistas.length) break;
            
            const c = dataCongresistas[currentIndex];
            const originalIdx = currentIndex; // capturar para el onclick
            const card = document.createElement("div");
            
            if(c.is_empty) {
                card.className = "curul-card empty-slot";
            } else {
                // Mapeo directo de clases para evitar errores de desincronización
                const mapClases = {
                    "A FAVOR": "favor",
                    "EN CONTRA": "contra",
                ...
                const cl = mapClases[c.voto_estado] || "pendiente";
                card.className = `curul-card ${cl}`;
                if (c.modificado) card.classList.add('modificado');
...
function toggleOralizado(event, index) {
    event.stopPropagation();
    dataCongresistas[index].oralizado = !dataCongresistas[index].oralizado;
    dataCongresistas[index].modificado = true;
    guardarEstadoLocal();
    renderHemiciclo();
}

/**
 * ANÁLISIS PRO: DISIDENCIAS Y BIPARTIDISMO
 */
...
function renderDisidencias() {
    const container = document.getElementById("disidencias-list");
    container.innerHTML = "";
    
    const bancadas = [...new Set(dataCongresistas.filter(c => !c.is_empty).map(c => c.bancada))];
    
    bancadas.forEach(b => {
        const membros = dataCongresistas.filter(c => c.bancada === b && c.voto_estado !== "PENDIENTE");
        if (membros.length === 0) return;
        
        // Calcular voto mayoritario
        const counts = {};
        membros.forEach(m => counts[m.voto_estado] = (counts[m.voto_estado] || 0) + 1);
        const mayoritario = Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b);
        
        // Encontrar disidentes
        const disidentes = membros.filter(m => m.voto_estado !== mayoritario);
        
        if (disidentes.length > 0) {
            const div = document.createElement("div");
            div.style.marginBottom = "10px";
            div.innerHTML = `<strong style="font-size:11px; color:#64748b;">${b} (Mayoría: ${mayoritario})</strong>`;
            disidentes.forEach(d => {
                div.innerHTML += `
                    <div style="display:flex; justify-content:space-between; font-size:11px; padding:5px; background:rgba(239, 68, 68, 0.1); border-radius:4px; margin-top:2px;">
                        <span>⚠️ ${d.nombre}</span>
                        <span style="color:var(--color-contra)">VOTO: ${d.voto_estado}</span>
                    </div>
                `;
            });
    ...
    if (container.innerHTML === "") {
        container.innerHTML = "<p style='text-align:center; opacity:0.5; padding:20px;'>No se detectan disidencias aún.</p>";
    }
}

function renderBipartidismo() {
    const container = document.getElementById("bipartidismo-container");
    container.innerHTML = "";
    
    // Coincidencias entre Bancadas
    const bancadas = ["FP", "PL", "APP", "AP", "RP", "PP"].filter(b => 
        dataCongresistas.some(c => c.bancada === b && c.voto_estado !== "PENDIENTE")
    );
    
    if (bancadas.length < 2) {
        container.innerHTML = "<p style='text-align:center; opacity:0.5; padding:20px;'>Faltan votos de múltiples bancadas para el radar.</p>";
        return;
    }

    let html = `<table style="width:100%; font-size:10px; border-collapse:collapse;"><tr><td></td>`;
    bancadas.forEach(b => html += `<td style="font-weight:bold; padding:5px; text-align:center;">${b}</td>`);
    html += "</tr>";

    bancadas.forEach(b1 => {
        html += `<tr><td style="font-weight:bold; padding:5px;">${b1}</td>`;
        bancadas.forEach(b2 => {
            if (b1 === b2) {
                html += `<td style="background:rgba(255,255,255,0.05);"></td>`;
            } else {
                // Calcular % de coincidencia
                const v1 = dataCongresistas.filter(c => c.bancada === b1 && c.voto_estado !== "PENDIENTE");
                const v2 = dataCongresistas.filter(c => c.bancada === b2 && c.voto_estado !== "PENDIENTE");
                
                if (v1.length > 0 && v2.length > 0) {
                    const mode1 = Object.entries(v1.reduce((a,c)=>{a[c.voto_estado]=(a[c.voto_estado]||0)+1;return a},{})).sort((a,b)=>b[1]-a[1])[0][0];
                    const mode2 = Object.entries(v2.reduce((a,c)=>{a[c.voto_estado]=(a[c.voto_estado]||0)+1;return a},{})).sort((a,b)=>b[1]-a[1])[0][0];
                    
                    const coincide = mode1 === mode2;
                    const style = coincide ? `background:rgba(16, 185, 129, 0.2); color:#10b981;` : `opacity:0.3;`;
                    html += `<td style="text-align:center; padding:5px; ${style}">${coincide ? 'SÍ' : 'NO'}</td>`;
                } else {
                    html += `<td style="text-align:center; opacity:0.2;">-</td>`;
                }
            }
        });
        html += "</tr>";
...
function updateStats() {
    const counts = { "A FAVOR": 0, "EN CONTRA": 0, "ABSTENCION": 0, "NO VOTO": 0, "MESA DIRECTIVA": 0, "LICENCIA": 0 };
    let verif = 0;
    
    dataCongresistas.forEach(c => {
        if(c.is_empty) return;
        const e = c.voto_estado;
        if(counts[e] !== undefined) counts[e]++;
        if(c.modificado) verif++;
    });
    
    // Actualizar números grandes
    const noVotaronTotal = (counts["NO VOTO"] || 0) + (counts["MESA DIRECTIVA"] || 0) + (counts["LICENCIA"] || 0);

    if (document.getElementById("count-favor")) document.getElementById("count-favor").innerText = counts["A FAVOR"];
    if (document.getElementById("count-contra")) document.getElementById("count-contra").innerText = counts["EN CONTRA"];
    if (document.getElementById("count-abstencion")) document.getElementById("count-abstencion").innerText = counts["ABSTENCION"];
    if (document.getElementById("count-novotaron")) document.getElementById("count-novotaron").innerText = noVotaronTotal;
    if (document.getElementById("verificados-count")) document.getElementById("verificados-count").innerText = verif;

    // Actualizar Barra de Progreso Meta
    const totalFavorMeta = counts["A FAVOR"] + counts["MESA DIRECTIVA"];
    const progress = Math.min((totalFavorMeta / currentMeta) * 100, 100);
    
    const bar = document.getElementById("meta-progress-bar");
    const pctLabel = document.getElementById("meta-percent");
    ...
    if (pctLabel) pctLabel.innerText = `${Math.round(progress)}% (${totalFavorMeta}/${currentMeta})`;
}

function actualizarVotoDesdeSelector(index, nuevoEstado) {
    dataCongresistas[index].voto_estado = nuevoEstado;
    dataCongresistas[index].modificado = true;
    
    // Actualizar clase de la tarjeta sin re-renderizar todo opcionalmente
    // Aquí re-renderizamos para simplificar, pero podríamos solo cambiar clase
    guardarEstadoLocal();
    renderHemiciclo(); 
    updateStats();
}
...
function nextState(index) {
    // Redundante con selector pero mantenemos lógica base
    const currentState = dataCongresistas[index].voto_estado;
    const currentIndex = ESTADOS.findIndex(e => e.id === currentState);
    const nextIndex = (currentIndex + 1) % ESTADOS.length;
    actualizarVotoDesdeSelector(index, ESTADOS[nextIndex].id);
}

function guardarEstadoLocal() {
    const backup = dataCongresistas.map(c => ({ dns: c.dni, v: c.voto_estado, m: c.modificado, o: c.oralizado }));
...
function restaurarEstadoLocal() {
    ...
    try {
        ...
        const map = new Map(saved.map(s => [s.dns, s]));
        dataCongresistas.forEach(c => {
            if (c.is_empty) return;
            const s = map.get(c.dni);
            if (s) {
                c.voto_estado = s.v;
                c.modificado = s.m;
                c.oralizado = s.o || false;
            }
        });
    } catch(e) { console.error(e); }
}

async function importarVotos(event) {
    ...
    try {
        ...
        const response = await fetch('/api/importar-votos', {
            method: 'POST',
            body: formData
        });
        const r = await response.json();
        ...
        if (r.data) {
            // Unir resultados (Mapeo por DNI)
            const mapImport = new Map(r.data.map(i => [str(i.dni), i]));
            dataCongresistas.forEach(c => {
                const imp = mapImport.get(str(c.dni));
                if (imp) {
                    c.voto_estado = imp.voto;
                    c.modificado = true;
                    if (imp.oralizado !== undefined) c.oralizado = imp.oralizado;
                }
            });
            renderHemiciclo();
            updateStats();
            guardarEstadoLocal();
            alert("✅ Importación exitosa: " + r.data.length + " votos procesados.");
        } else {
            alert("⚠️ Error: " + (r.error || "No se pudieron procesar los datos."));
        }
        btn.innerText = originalText;
    } catch(e) {
        console.error(e);
        alert("❌ Error crítico en la importación.");
    }
}

// Auxiliar para asegurar string
...
function renderHeatmap() {
    ...
    bancadas.forEach(b => {
        const miembros = dataCongresistas.filter(c => c.bancada === b);
        const favor = miembros.filter(m => m.voto_estado === "A FAVOR").length;
        const contra = miembros.filter(m => m.voto_estado === "EN CONTRA").length;
        const abs = miembros.filter(m => m.voto_estado === "ABSTENCION").length;
        const total = miembros.length;
        const pct = (favor / total) * 100;
        
        const card = document.createElement("div");
        card.className = "analysis-card";
        card.onclick = () => verDetalleBancada(b); // Nueva interacción
        
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between; margin-bottom:12px;">
                <strong style="font-size:14px; letter-spacing:1px;">${b}</strong>
                <span style="font-size:11px; opacity:0.6;">${favor}/${total} VOTOS</span>
            </div>
            <div style="margin-bottom:8px; font-size:10px; display:flex; gap:10px; opacity:0.8;">
                <span style="color:var(--color-favor)">FAV: ${favor}</span>
                <span style="color:var(--color-contra)">CON: ${contra}</span>
                <span style="color:var(--color-abstencion)">ABS: ${abs}</span>
            </div>
            <div style="height:6px; background:#111; border-radius:3px; overflow:hidden; border:1px solid #222;">
                <div style="width:${pct}%; height:100%; background:var(--color-favor); box-shadow: 0 0 12px var(--color-favor); transition: width 1s;"></div>
            </div>
        `;
...
function verDetalleBancada(bancada) {
    ...
    title.innerText = `DETALLE NOMINAL: ${bancada}`;
    body.innerHTML = "";
    
    const miembros = dataCongresistas.filter(c => c.bancada === bancada && !c.is_empty);
    // Ordenar: Favor primero, luego Contra, luego Abstencion, etc.
    const orden = {"A FAVOR":1, "EN CONTRA":2, "ABSTENCION":3, "PENDIENTE":4};
    miembros.sort((a,b) => (orden[a.voto_estado]||9) - (orden[b.voto_estado]||9));
...
function renderMapa() {
    ...
    const validData = dataCongresistas.filter(c => !c.is_empty);
    const regionesValidas = [...new Set(validData.map(c => c.region.toUpperCase()))];

    // Renderizar cada departamento
    Object.keys(PERU_PATHS).forEach(regId => {
        const pathData = PERU_PATHS[regId];
        const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
        path.setAttribute("d", pathData);
        
        // Calcular Porcentaje de Apoyo
        const miembros = validData.filter(c => c.region.toUpperCase() === regId);
        let color = "#1e293b"; // Default empty
        let info = `${regId}: Sin representantes`;

        if (miembros.length > 0) {
            const favor = miembros.filter(m => m.voto_estado === "A FAVOR").length;
            const pct = (favor / miembros.length) * 100;
            // Heatmap Interpolation (Verde si pct > 0)
            color = `rgba(16, 185, 129, ${0.1 + (pct/120)})`;
            info = `${regId}: ${favor}/${miembros.length} APOYO (${Math.round(pct)}%)`;
        }
        ...
        path.onmouseover = () => { path.setAttribute("fill", "#10b981"); };
        path.onmouseout = () => { path.setAttribute("fill", color); };
    ...
    // EXTRANJERO como punto separado fuera de la costa
    const extMiembros = validData.filter(c => c.region.toUpperCase() === "EXTRANJERO");
    if (extMiembros.length > 0) {
        const extDiv = document.createElement("div");
        const favor = extMiembros.filter(m => m.voto_estado === "A FAVOR").length;
        extDiv.style.position = "absolute";
        extDiv.style.bottom = "20px";
        extDiv.style.left = "20px";
        extDiv.style.padding = "10px";
        extDiv.style.background = "rgba(255,255,255,0.1)";
        extDiv.style.border = "1px solid #333";
        extDiv.style.borderRadius = "8px";
        extDiv.innerHTML = `<div style="font-size:9px; opacity:0.7;">EXTRANJERO</div><strong>${favor}/${extMiembros.length}</strong>`;
        container.appendChild(extDiv);
    }
}

/**
 * MAPA CARTESIANO POR BANCADA (Sin solapamientos)
 */
function renderIndividualPoints() {
    const container = document.getElementById("scatter-dots");
    if (!container) return;
    container.innerHTML = "";
    
    // Etiquetas de Cuadrantes
    const labels = [
        { t: "EN CONTRA", x: 25, y: 8 },
        { t: "A FAVOR", x: 75, y: 8 },
        { t: "ABSTENCIÓN", x: 25, y: 58 },
        { t: "NO VOTO / OTROS", x: 75, y: 58 }
    ];
    labels.forEach(l => {
        const div = document.createElement("div");
        div.innerText = l.t;
    ...
    const partyColors = {
        "FP": "#ff8000", "PL": "#e30613", "APP": "#0055a6",
        "AP": "#ffff00", "RP": "#00aae4", "PP": "#4b2e83",
        "BM": "#ffffff", "ID": "#10b981", "SP": "#fbbf24",
        "HA": "#ec4899", "BS": "#00d1ff", "JPP": "#00d1b2",
        "AV.P": "#ff0080", "HD": "#ff4500", "BDP": "#8a2be2",
        "N.A.": "#475569", "JP": "#00d1b2" // N.A. ahora en Pizarra para evitar confusión
    };

    const validData = dataCongresistas.filter(c => !c.is_empty);
    
    // Agrupar por cuadrante para centrado dinámico
    const quads = { "f": [], "c": [], "a": [], "o": [] };
    validData.forEach(c => {
        if (c.voto_estado === "A FAVOR") quads.f.push(c);
        else if (c.voto_estado === "EN CONTRA") quads.c.push(c);
        else if (c.voto_estado === "ABSTENCION") quads.a.push(c);
        else quads.o.push(c);
    });

    const renderQuad = (list, qX, qY) => {
        if (list.length === 0) return;
        
        // Calcular dimensiones de la cuadrícula necesaria
        const cols = Math.ceil(Math.sqrt(list.length));
        const spacing = 4.0; // Espaciado entre puntos (%)
        
        // Centrar el bloque dentro del cuadrante (50% x 50%)
        // Margen de seguridad del 10% dentro del cuadrante (5% cada lado)
        const blockW = (cols - 1) * spacing;
        const startX = (qX * 50) + (50 - blockW) / 2;
        const startY = (qY * 50) + (50 - blockW) / 2;
        ...
        list.forEach((c, i) => {
            ...
            const posX = startX + (col * spacing) + jitterX;
            const posY = startY + (row * spacing) + jitterY;
            
            const dot = document.createElement("div");
            const color = partyColors[c.bancada] || "#64748b";
            const shortName = formatNombreOficial(c.nombre);

            dot.className = "congress-dot";
            ...
            if (c.foto_url) {
                ...
                img.src = c.foto_url;
                img.className = "congress-hover-photo";
                label.appendChild(img);
            }
            
            const spanName = document.createElement("span");
            spanName.innerText = shortName;
            spanName.style.textShadow = "1px 1px 2px #000";
            ...
            dot.title = `${c.nombre} (${c.bancada})`;
            container.appendChild(dot);
        });
    };

    renderQuad(quads.f, 1, 0); // A FAVOR (Der-Arriba)
    renderQuad(quads.c, 0, 0); // EN CONTRA (Izq-Arriba)
    renderQuad(quads.a, 0, 1); // ABSTENCION (Izq-Abajo)
    renderQuad(quads.o, 1, 1); // OTROS (Der-Abajo)

    renderCartesianLegend(partyColors);
}

function renderCartesianLegend(colors) {
    const legend = document.getElementById("cartesian-legend-outer");
    if(!legend) return;
    
    const bs = [...new Set(dataCongresistas.filter(c => !c.is_empty).map(c => c.bancada))];
    legend.innerHTML = `
        <h4 style="font-size:12px; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.2); padding-bottom:8px; text-align:center; letter-spacing:2px; font-weight:800;">DISTRIBUCIÓN POLÍTICA POR BANCADA</h4>
        <div id="leg-grid" style="display:flex; flex-wrap:wrap; justify-content:center; gap:12px;"></div>
    `;
    const grid = legend.querySelector("#leg-grid");
    bs.forEach(b => {
        const item = document.createElement("div");
        item.style.display = "flex"; item.style.alignItems = "center"; item.style.gap = "8px"; item.style.fontSize = "10px"; item.style.fontWeight = "600";
        const d = document.createElement("span");
        d.style.width = "9px"; d.style.height = "9px"; d.style.background = colors[b] || "#888"; d.style.borderRadius = "50%";
        item.appendChild(d); item.appendChild(document.createTextNode(b));
